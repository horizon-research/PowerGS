    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if pruning_counter % args.prune_interval == 0 and pruning_counter < args.prune_iterations:
                ...
                gaussians, pass_test, quality = controllable_prune(gaussians, scene, pipe, bg, dataset, args, prune_num, ouput_folder, hvs=False, pooling_size=1)
                log_message(f'[{iteration}] Pruning: {pass_test}, Pnum: {gaussians.get_xyz.shape[0]}, SSIM: {quality["ssim"]}, PSNR: {quality["psnr"]}', log_path)
            if pruning_counter == args.prune_iterations:
                print(f"\n[ITER {iteration}] Final Test")
                gaussians, pass_test, quality, restore_failed = final_test(gaussians, scene, pipe, bg, dataset, args, opt, ouput_folder, hvs=False, pooling_size=1)
                log_message(f'[{iteration}] Final Test: {pass_test}, Pnum: {gaussians.get_xyz.shape[0]}, SSIM: {quality["ssim"]}, PSNR: {quality["psnr"]}', log_path)
                if restore_failed:
