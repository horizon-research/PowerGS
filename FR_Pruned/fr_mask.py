...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    ## 1 - Decide Prune number every prune_interval
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if pruning_counter % args.prune_interval == 0 and pruning_counter < args.prune_iterations:
                ...
                gaussians, pass_test, quality = controllable_prune(gaussians, scene, pipe, bg, dataset, args, prune_num, ouput_folder, hvs=True, pooling_size=args.pooling_size)
                log_message(f'[{iteration}] Pruning: {pass_test}, Pnum: {gaussians.get_xyz.shape[0]}, HVS: {quality["hvs"]}, PS: {args.pooling_size}', log_path)
            if pruning_counter == args.prune_iterations:
                print(f"\n[ITER {iteration}] Final Test")
                gaussians, pass_test, quality, restore_failed = final_test(gaussians, scene, pipe, bg, dataset, args, opt, ouput_folder, hvs=True, pooling_size=args.pooling_size)
                log_message(f'[{iteration}] Final Test: {pass_test}, Pnum: {gaussians.get_xyz.shape[0]}, HVS: {quality["hvs"]}, PS: {args.pooling_size}', log_path)
                if restore_failed:
                    log_message(f'[{iteration}] -------- Restore failed  -------- : Never meet target.', log_path)
            pruning_counter += 1
