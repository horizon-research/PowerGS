...
from utils.display_power import display_power, display_power_metric_boost
import torch
...
from utils.controllable_pruning import controllable_prune, final_test, test_ssim_loss, test_psnr_loss, test_display_power, test_render_power
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    per_finetune_budget = 10000 # 10000 x relative_prune_ratio
    min_iter = 1500
    first_iter = 5000
    ...
    for idx in range(coarse_search_num):
        ...
        log_message(f"SSIM: {tested_ssim:.4f}", log_path)
        log_message(f"PSNR: {tested_psnr:.4f}", log_path)
        ...
        coarse_ssims.append(tested_ssim.item())
        coarse_psnrs.append(tested_psnr.item())
    ...
    # log ratio and prune ratio
    log_message(f"Fine search ratio: {fine_prune_ratio}", log_path)
    ...
    for idx in range(fine_search_num):
        ...
        if prune_num > coarse_prune_num:
            ...
            # print the shift
            log_message(f"Fine search idx: {idx}, Shift model from {start_idx} to {start_idx+1}", log_path)
        # if gaussians.get_xyz.shape[0] > prune_num:
        gaussians, pass_test, quality = controllable_prune(gaussians, scene, pipe, background, dataset, args, prune_num,
                                                        ouput_folder, hvs=False, pooling_size=1, 
        ...
        fine_ssims.append(tested_ssim.item())
        fine_psnrs.append(tested_psnr.item())
    ...
    for idx, (power, passes) in enumerate(zip(coarse_total_powers, coarse_passes)):
        if passes:
            ...
            all_metrics.append({
                ...
                "ssim": coarse_ssims[idx],
                "psnr": coarse_psnrs[idx]
            })
    ...
    log_message(f"SSIM: {best_metrics['ssim']:.4f}", log_path)
    log_message(f"PSNR: {best_metrics['psnr']:.4f}", log_path)
