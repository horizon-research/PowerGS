	...
	bool debug)
{
	...
	// float t_before_preprocessing = 0;
	// cudaEventElapsedTime(&t_before_preprocessing, start, stop);
	...
	// cudaEventRecord(stop);
	// cudaEventSynchronize(stop);
	// float t_preprocess = 0;
	// cudaEventElapsedTime(&t_preprocess, start, stop);
	// --- End: preprocessing --- //
	...
	// std::cout << "Number of rendered gaussians (BEFORE): " << num_rendered << std::endl;
	...
	// cudaEventSynchronize(stop);
	// float t_pre_obb = 0;
	// cudaEventElapsedTime(&t_pre_obb, start, stop);
	// --- End: PreOBB --- //
	...
	// cudaEventSynchronize(stop);
	// float t_obb = 0;
	// cudaEventElapsedTime(&t_obb, start, stop);
	// --- End: OBB --- //


	// ---- Start Compute Colors ---- //
	// cudaEventRecord(start);
	compute_colors << <(P + 255) / 256, 256 >> >  (
		P, D, M, means3D, (glm::vec3*)cam_pos, shs,
		radii,
		geomState.rgb);
	// cudaEventRecord(stop);
	// cudaEventSynchronize(stop);
	// float t_compute_colors = 0;
	// cudaEventElapsedTime(&t_compute_colors, start, stop);
	...
	// cudaEventRecord(stop);
	// cudaEventSynchronize(stop);
	// float t_pre_duplicate = 0;
	// cudaEventElapsedTime(&t_pre_duplicate, start, stop);
	// End Pre Duplicating
	...
	// cudaEventRecord(stop);
	// cudaEventSynchronize(stop);
	// float t_duplicating = 0;
	// cudaEventElapsedTime(&t_duplicating, start, stop);
	// --- End: Duplicating --- //
	...
	// cudaEventRecord(stop);
	// cudaEventSynchronize(stop);
	// float t_pre_sorting = 0;
	// cudaEventElapsedTime(&t_pre_sorting, start, stop);
	// --- End: Pre Sorting --- //
	...
	// cudaEventSynchronize(stop);
	// float t_sorting = 0;
	// cudaEventElapsedTime(&t_sorting, start, stop);
	// --- End: Sorting --- //
	...
	// cudaEventRecord(stop);
	// cudaEventSynchronize(stop);
	// float t_pre_identify_tile_ranges = 0;
	// cudaEventElapsedTime(&t_pre_identify_tile_ranges, start, stop);
	...
	// cudaEventRecord(stop);
	// cudaEventSynchronize(stop);
	// float t_identify_tile_ranges = 0;
	// cudaEventElapsedTime(&t_identify_tile_ranges, start, stop);
	...
	// cudaEventSynchronize(stop);
	// float t_rendering = 0;
	// cudaEventElapsedTime(&t_rendering, start, stop);
