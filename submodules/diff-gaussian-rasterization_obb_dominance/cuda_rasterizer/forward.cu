	...
	float* __restrict__ contributions)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			atomicAdd(&gaussians_count[coll_id], 1);
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (contrib > max_point_contrib)
			{
				max_point_contrib = contrib;
				max_point_idx = collected_id[j];
			}

			// atomicAdd(&contributions[collected_id[j]], alpha * T);
	...
	if (inside)
	{
		atomicAdd(&contributions[max_point_idx], 1);
		final_T[pix_id] = T;
