	...
	)
{
	...
	float3 p_view;                                                                                                                                                   
	bool in_frust = in_frustum(idx, orig_points, viewmatrix, projmatrix, prefiltered, p_view);                                                                       
																																									
	// Write culling result                                                                                                                                          
	per_point_cull[idx] = in_frust ? 1 : 0;                                                                                                                            
		...
		int* per_point_used)
{
	...
	__shared__ RenderInfo2 render_infos2[BLOCK_SIZE];
	...
	int T_inter = (int) (range.y - range.x);	
	int T_load = 0;
	int T_bf_term = 0;
	int T_extra_alpha = 0;
	int T_used1 = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			skip_info1.opacity1 = opacities[coll_id * fov_num + L1_i];
			skip_infos1[block.thread_rank()] = skip_info1;

			RenderInfo1 render_info1;
			render_info1.features = features[coll_id * fov_num + L1_i];
			render_infos1[block.thread_rank()] = render_info1;

			EarlySkipInfo2 skip_info2;
			skip_info2.opacity2 = opacities[coll_id * fov_num + L2_i];
			skip_info2.highest_level = highest_levels[coll_id];
			skip_infos2[block.thread_rank()] = skip_info2;

			RenderInfo2 render_info2;
			render_info2.features = features[coll_id * fov_num + L2_i];
			render_infos2[block.thread_rank()] = render_info2;
			per_point_used[coll_id] = 1;

		}
		block.sync();
		T_load +=  min(BLOCK_SIZE, toDo);
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (!L2_done)
			{
				...
				float alpha2 = min(0.99f, skip_info2.opacity2 *exp_val);
				bool alpha_skip_2 = alpha2 < 1.0f / 255.0f;
				bool skip2 = alpha_skip_2 || ( (skip_info2.highest_level + 1) < L2_f);
				if (!skip2){
					float test_T2 = T2 * (1 - alpha2);
					L2_done = (test_T2 < 0.0001f);
					if (!L2_done)
					{
						T_used2++;
						RenderInfo2 render_info2 = render_infos2[j];
						float w = alpha2 * T2;
						C2[0] += render_info2.features.x * w;
						C2[1] += render_info2.features.y * w;
						C2[2] += render_info2.features.z * w;
						T2 = test_T2;
					}
				}
			}
	...
	if (inside)
	{
		...
		pixel_statistics[0 * H * W + pix_id] = T_inter; // T_inter
		pixel_statistics[1 * H * W + pix_id] = T_load; // Tload
		pixel_statistics[2 * H * W + pix_id] = T_bf_term; // Tbf_term
		pixel_statistics[3 * H * W + pix_id] = T_extra_alpha; // T_extra_alpha
		pixel_statistics[4 * H * W + pix_id] = T_used1; // Tused1
		pixel_statistics[5 * H * W + pix_id] = T_used2; // Tused2
	}
}
		...
		int* per_point_used)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			skip_info.opacity1 = opacities[coll_id * fov_num + tile_level_i];
			skip_infos[block.thread_rank()] = skip_info;

			RenderInfo render_info;
			render_info.features = features[coll_id * fov_num + tile_level_i];
