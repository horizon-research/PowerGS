...
__device__ glm::vec3 computeRestColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs)
{
	...
	if (deg > 0)
	{
		...
		float y = dir.y;
		float z = dir.z;
		result = result - SH_C1 * y * sh[1 - 1] + SH_C1 * z * sh[2 - 1] - SH_C1 * x * sh[3 - 1];
...
__global__ void filter(
	...
	const float* highest_levels, //for FOV
	const float* tile_levels, //for FOV
	const bool* tile_blendings,
	uint32_t* tiles_touched,
	bool* filter_result,
	const int tile_width_num,
	uint2* level_ranges,
	int* per_point_inter,
	int* per_point_n_iter,
	int* per_point_obb_count,
	int* 	P_tile_ecc_ptr,
	int* p_levels)
{
	auto idx = cg::this_grid().thread_rank();
	...
	if (radii[idx] > 0)
	{
		...
		uint count=0;
		float highest_level = highest_levels[idx];
		float lowest_level_used = highest_level;
		float highest_level_used = 0;
		bool be_blending = false;

		
		if (potential_tnum == 1)
		{
			uint tile_idx =  rect_min.y  * tile_width_num + rect_min.x;
			float level = tile_levels[tile_idx];
			P_tile_ecc_ptr[idx] += 1;
			bool blending = tile_blendings[tile_idx];
			if ( level < (highest_level + 1) ) 
			{
				count = 1;
				lowest_level_used = level;
				highest_level_used = level;
				be_blending = blending || be_blending;
			...
			for (int y = rect_min.y; y < rect_max.y; y++)
			{
				for (int x = rect_min.x; x < rect_max.x; x++)
				{
					...
					if (inside)
					{
						...
						{
							count++;
							lowest_level_used = min(lowest_level_used, level);
							highest_level_used = max(highest_level_used, level);
							be_blending = blending || be_blending;
						}
					}
		...
		per_point_n_iter[idx] = count;
		if (count == 0)
		{
			radii[idx] = 0;
		}
		else{
			per_point_inter[idx] = 1;
			level_ranges[idx].x = int(lowest_level_used);
			int highest_level_used_i = int(highest_level_used);
			if (be_blending){
				highest_level_used_i = min(highest_level_used_i + 1, fov_num - 1);
			}
			level_ranges[idx].y = highest_level_used_i;

			p_levels[idx] = highest_level_used_i - int(lowest_level_used);
		}
	}
}
...
__global__ void compute_fov_colors(
	...
	const uint2* __restrict__ level_ranges,
	const float* __restrict__ shs_dcs,
	float3* __restrict__ fov_colors)
{
	auto idx = cg::this_grid().thread_rank();
	if (idx >= P)
		return;
	...
	if (radii[idx] > 0)
	{
		...
		uint2 range = level_ranges[idx];

		for (int layer_i = range.x; layer_i <= range.y; layer_i++)
		{
			float3 color = { 0.0f, 0.0f, 0.0f };
			float dc1 = shs_dcs[idx * 3 * fov_num + layer_i * 3 + 0]; 
			float dc2 = shs_dcs[idx * 3 * fov_num + layer_i * 3 + 1];
			float dc3 = shs_dcs[idx * 3 * fov_num + layer_i * 3 + 2];

			color.x = SH_C0 * dc1 + result.x;
			color.y = SH_C0 * dc2 + result.y;
			color.z = SH_C0 * dc3 + result.z;
			...
			fov_colors[idx * fov_num + layer_i] = color;
		}
...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P)
{
	...
	obtain(chunk, geom.level_ranges, P, 128);
	return geom;
}
...
int CudaRasterizer::Rasterizer::forward(
	...
	float* eigen_lengths,
	float* eigen_vecs,
	int* point_statistics,
	int* pixel_statistics,
	bool debug)
{


	int* P_cull_ptr = point_statistics + 0 * P;  
	int* N_inter_ptr = point_statistics + 1 * P;  
	int* P_inter_ptr = point_statistics + 2 * P; 
	int* P_used_ptr = point_statistics + 3 * P;  
	int* P_obb_count_ptr = point_statistics + 4 * P;
	int* P_tile_ecc_ptr = point_statistics + 5 * P;
	int* P_levels = point_statistics + 6 * P;
	...
	size_t fov_color_size =  P * fov_num * sizeof(float3);
	...
	compute_fov_colors << <(P + 255) / 256, 256 >> >  (
		P, D, M, means3D, (glm::vec3*)cam_pos, shs_rest,
		radii,
		geomState.level_ranges,
		shs_dcs,
		fov_colors);
