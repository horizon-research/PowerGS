...
export method_name="FR_prune_"$q_required""
...
if is_in_list "$scene_name" "${mipnerf360_outdoor_scenes[@]}"; then
  ...
  python FR_Pruned/fr_prune_L1.py -s "$m360_dataset_base_dir/$scene_name" -m "/workspace/PowerGS/models/$method_name/m360/$scene_name/levels/L1" -i images_4 --eval \
  --checkpoint_iterations 60000 --prune_ratio 0.05 --prune_interval 1000 --prune_iterations 25000 --q_required "$q_required" \
  --start_checkpoint "/workspace/PowerGS/models/mini-splatting-d/m360/"$scene_name"/chkpnt30000.pth" --port 1111
  for i in "${!PSs[@]}"; do
      echo "Running iteration $((i+1)) of ${#PSs[@]}"
      python FR_Pruned/fr_mask.py \
  ...
  python FR_Pruned/fr_prune_L1.py \
    -s "$m360_dataset_base_dir/$scene_name" \
  ...
  python FR_Pruned/fr_prune_L1.py \
    ...
    --prune_iterations 25000 \
    --q_required "$q_required" \
    --start_checkpoint "/workspace/PowerGS/models/3dgs/nerf_synthetic/$scene_name/chkpnt30000.pth" \
    --port 6666
  ...
  python FR_Pruned/fr_prune_L1.py \
    ...
    --prune_iterations 25000 \
    --q_required "$q_required" \
    --start_checkpoint "/workspace/PowerGS/models/3dgs/extend/$scene_name/chkpnt30000.pth" \
    --port 6666
