...
if is_in_list "$scene_name" "${mipnerf360_outdoor_scenes[@]}"; then
  ...
  python FR_Ours/final_prune_L1.py -s "$m360_dataset_base_dir/$scene_name" -m "/workspace/PowerGS/models/$method_name/m360/$scene_name/levels/L1" -i images_4 --eval \
  ...
  --start_checkpoint "/workspace/PowerGS/models/mini-splatting-d/m360/"$scene_name"/chkpnt30000.pth" --port 1212
  for i in "${!PSs[@]}"; do
  ...
  python  FR_Ours/final_prune_L1.py \
    -s "$m360_dataset_base_dir/$scene_name" \
    -m "/workspace/PowerGS/models/$method_name/m360/$scene_name/levels/L1" \
    -i images_2 \
    --eval \
    --checkpoint_iterations 60000 \
    --prune_interval 1000 --prune_iterations 15000 --q_required "$q_required" --adjust_interval 200 --adjust_iterations 10000 \
