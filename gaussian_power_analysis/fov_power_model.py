...
def fov_hardware_power_modeling(
    ...
    Args:
        point_statistics (torch.Tensor): shape (7, P), containing:
            [ Pcull, N_inter, P_inter, P_used, P_obb_count, P_Tile_ECC_accesses_count, P_levels ] per point.
        pixel_statistics (torch.Tensor): shape (6, H, W), containing:
            [ Tinter, Tload, Tbf_term, T_extra_alpha, Tused1, Tused2] per pixel.
    ...
    P_Tile_ECC_accesses_count = point_statistics[5].sum().item()
    ...
    pixel_stats_tiles = pixel_stats_tiles.reshape(6, H_pad // 16, W_pad // 16, 16*16).float()
    ...
    # #used    = sum(Tused)
    P_used = point_statistics[5]
    P_levels =  point_statistics[6] + 1
    P_levels_masked = P_levels * P_used
    Pused1_num = (P_levels_masked==1).sum().item()
    Pused2_num = (P_levels_masked==2).sum().item()
    Pused3_num = (P_levels_masked==3).sum().item()
    Pused4_num = (P_levels_masked==4).sum().item()


    #  [ Tinter, Tload, Tbf_term, T_extra_alpha, Tused1, Tused2] per pixel.
    bf_term_sum = pixel_statistics[2].sum().item()
    pix_extra_alpha_sum = pixel_statistics[3].sum().item()
    pix_used1_sum = pixel_statistics[4].sum().item()
    pix_used2_sum = pixel_statistics[5].sum().item()
    ...
    rendering_dram = (
        (T_load * 19).sum().item()
        + Pused1_num * 103 + Pused2_num *(90 + 19 * 2) + Pused3_num * (90 + 19 * 3) + Pused4_num * (90 + 19 * 4)
        + (H * W * 3)
    )
    ...
    rendering_flops = (
        (FLOPs_SHs * Pused1_num) + Pused2_num * (FLOPs_SHs + 6) + Pused3_num * (FLOPs_SHs + 9) + Pused4_num * (FLOPs_SHs + 12)
        + (FLOPs_CompAlpha * bf_term_sum) + (pix_extra_alpha_sum)
        + (FLOPs_InterColor * (pix_used1_sum + pix_used2_sum))
