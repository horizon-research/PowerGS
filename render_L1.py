...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, base_folder):
    render_path = os.path.join(base_folder, "L1", name, "ph", "renders")
    gts_path = os.path.join(base_folder, "L1", name, "ph", "gt")
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, base_folder):
    with torch.no_grad():
        ...
        if ckpt_name is None:
            ckpt_name = find_ckpt(folder = os.path.join(base_folder))
            if ckpt_name is None:
                raise FileNotFoundError(f"No checkpoint found in {base_folder}")
            else:
                ckpt_path = os.path.join(os.path.join(base_folder), ckpt_name)
        else:# Restore the model from the checkpoint
        ...
        if not skip_train:
             render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, base_folder)

        if not skip_test:
             render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, base_folder)
